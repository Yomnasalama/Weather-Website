async function search (res) {
    let t = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=d096a1832c31457d838202052240312&q=${res}&days=3`);
    if (t.ok && 400 != t.status) {
        var res = await t.json();
...
document.getElementById("search").addEventListener("keyup", a => {
    ...
    console.log("yoyo", a.target.value)
}
);
